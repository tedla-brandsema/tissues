...
FIRESTORE_DATABASE_ID="tissues-native"
REGION="europe-west4"
...
TISSUES_BOOTSTRAP_TENANT_ID="7womw3jzkek74oggxj6f42xak4"
...
CLIENT_METADATA_URL_LIST="${TISSUES_AUTH_CLIENT_METADATA_URL_LIST-}"
unset TISSUES_IDENTITY_API_KEY
unset TISSUES_AUTH_CLIENT_METADATA_URL_LIST

if [[ "${CLIENT_METADATA_URL_LIST}" == *,* ]]; then
  echo "TISSUES_AUTH_CLIENT_METADATA_URL_LIST must not contain commas; use semicolons between Client ID Metadata Document URLs." >&2
  exit 2
fi
...
echo "Firestore database           : ${FIRESTORE_DATABASE_ID} (prepared target; created in F6)"
echo "Production asset bucket      : ${TISSUES_ASSET_BUCKET}"
echo "Production bootstrap tenant  : ${TISSUES_BOOTSTRAP_TENANT_ID}"
echo "----------------------------------------"
...
NON_SECRET_ENV="TISSUES_SERVER_READ_TIMEOUT=60s,TISSUES_SERVER_WRITE_TIMEOUT=60s,TISSUES_FIRESTORE_PROJECT_ID=${PROJECT_ID},TISSUES_FIRESTORE_DATABASE_ID=${FIRESTORE_DATABASE_ID},TISSUES_AUTH_ENABLED=true,TISSUES_AUTH_ISSUER_URL=${PUBLIC_ORIGIN},TISSUES_AUTH_MCP_RESOURCE_URL=${PUBLIC_ORIGIN}/mcp,TISSUES_AUTH_CLIENT_ID=tissues,TISSUES_AUTH_CLIENT_REDIRECT_URI=${PUBLIC_ORIGIN}/tissues/auth/callback,TISSUES_AUTH_CLIENT_METADATA_URL_LIST=${CLIENT_METADATA_URL_LIST},TISSUES_AUTH_INSECURE_COOKIE=false,TISSUES_TISSUES_ENABLED=true,TISSUES_TISSUES_BOOTSTRAP_TENANT_ID=${TISSUES_BOOTSTRAP_TENANT_ID},TISSUES_TISSUES_ASSETS_BUCKET=${TISSUES_ASSET_BUCKET},TISSUES_TISSUES_AUTH_ENABLED=true,TISSUES_TISSUES_AUTH_BROKER_URL=${PUBLIC_ORIGIN},TISSUES_TISSUES_AUTH_CLIENT_ID=tissues,TISSUES_TISSUES_AUTH_REDIRECT_URI=${PUBLIC_ORIGIN}/tissues/auth/callback,TISSUES_TISSUES_AUTH_INSECURE_COOKIE=false"
